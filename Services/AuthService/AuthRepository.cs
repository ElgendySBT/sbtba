...
namespace SBTBackEnd.Services.AuthService
{
	public class AuthRepository : IAuthRepository
	{
		...
		{
			var user = await _context.SBTUsers.FirstOrDefaultAsync(x => x.Username == userName);
			if (user ==null )
			{
				return null;
			}
		...
		{
			if (await _context.SBTUsers.AnyAsync(x=>x.Username == userName) )
			{
				return true;
			}
			return false;
